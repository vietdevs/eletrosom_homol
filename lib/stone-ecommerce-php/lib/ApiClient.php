...
class ApiClient
{
    ...
    public function createSale(One\DataContract\Request\CreateSaleRequest $createSaleRequest)
    {
        ...
        if (empty($createSaleResponse->BoletoTransactionResultCollection) && empty($createSaleResponse->CreditCardTransactionResultCollection)) {
            ...
            if (count($createSaleResponse->BoletoTransactionResultCollection) > 0) {
                foreach ($createSaleResponse->BoletoTransactionResultCollection as $boletoTransaction) {
                    if (!$boletoTransaction->Success) {
                        $isSuccess = false;
                    }
                }
            }

            if (count($createSaleResponse->CreditCardTransactionResultCollection) > 0) {
