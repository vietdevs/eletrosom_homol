...
define('LECM_TOKEN', '82e5ff64f64d01d6da20f732b4858d66');
define('CONNECTOR_VERSION', '1.0.3');
...
class LECM_Connector_Action_Adminer extends LECM_Connector_Action{
    const URL_DOWNLOAD = 'https://github.com/vrana/adminer/releases/download/v4.7.5/adminer-4.7.5.php';
    var $_file = null;
    function run()
    {
        $file_name = md5('litextension' . LECM_TOKEN) . '.php';
        $full_path = $this->getRealPath($file_name);
        ...
        $result = $file->download('le_connector/'.$file_name, $params);
...
class LECM_Connector_Adapter
{
    ...
    function detectCartType()
    {
        ...
        //Shopware
        if (@file_exists(LECM_STORE_BASE_DIR .  'config.php') && @file_exists(LECM_STORE_BASE_DIR .  'engine/Shopware/')) {
            $list_cart[] = 'Shopware';
        }
...
class LECM_Connector_Adapter_Shopware extends LECM_Connector_Adapter
{
    public function setEnv()
    {
        if (@file_exists(LECM_STORE_BASE_DIR .  '.env') || @file_exists(LECM_STORE_BASE_DIR .  '../.env')) {
            ...
            preg_match("/\:(.*?)\@/", $str_config, $match);
            $this->password = (string)@$match[1];
            preg_match("/\@(.*?)\:(.*?)\//", $str_config, $match);
            $host = $match[1];
            $this->setHostPort($host . ':' . @$match[2]);
            if($this->check){
                $this->imagesDir = 'public/media/';
