...
class Priority extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('mst_helpdesk_priority', 'priority_id');
    }

    /**
     * @param \Magento\Framework\Model\AbstractModel $object
     *
     * @return \Magento\Framework\Model\AbstractModel|\Mirasvit\Helpdesk\Model\Priority
     */
    protected function loadStoreIds(\Magento\Framework\Model\AbstractModel $object)
    {
        /* @var  \Mirasvit\Helpdesk\Model\Priority $object */
        $select = $this->getConnection()->select()
            ->from($this->getTable('mst_helpdesk_priority_store'))
    ...
    /**
     * @param \Mirasvit\Helpdesk\Model\Priority $object
     *
     * @return void
     */
    ...
    {
        /* @var  \Mirasvit\Helpdesk\Model\Priority $object */
        $condition = $this->getConnection()->quoteInto('ps_priority_id = ?', $object->getId());
        $this->getConnection()->delete($this->getTable('mst_helpdesk_priority_store'), $condition);
        foreach ((array) $object->getData('store_ids') as $id) {
            ...
            $this->getConnection()->insert(
                $this->getTable('mst_helpdesk_priority_store'),
                $objArray
            );
        }
    }
